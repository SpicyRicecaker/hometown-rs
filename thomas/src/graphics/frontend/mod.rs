...
impl State {
    ...
    /// We should probably use cgmath vectors, there's too many parameters here
    /// Or we could do calculation on the shader
    pub fn draw_line_3d(&mut self, x1: f32, y1: f32, z1: f32, x2: f32, y2: f32, z2: f32, thickness: f32, color: Color) {
        let color = wgpu::Color::from(color);
        let color = [
            color.r as f32,
            color.g as f32,
